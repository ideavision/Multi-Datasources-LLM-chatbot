...
from payserai.configs.payseraibot_configs import PAYSERAI_BOT_ANSWER_GENERATION_TIMEOUT
from payserai.configs.payseraibot_configs import PAYSERAI_BOT_DISABLE_DOCS_ONLY_ANSWER
from payserai.configs.payseraibot_configs import PAYSERAI_BOT_DISPLAY_ERROR_MSGS
from payserai.configs.payseraibot_configs import PAYSERAI_BOT_NUM_RETRIES
from payserai.configs.payseraibot_configs import PAYSERAI_REACT_EMOJI
from payserai.configs.payseraibot_configs import DISABLE_PAYSERAI_BOT_FILTER_DETECT
from payserai.configs.payseraibot_configs import ENABLE_PAYSERAIBOT_REFLEXION
from payserai.connectors.slack.utils import make_slack_api_rate_limited
from payserai.payseraibot.slack.blocks import build_documents_blocks
from payserai.payseraibot.slack.blocks import build_qa_response_blocks
from payserai.payseraibot.slack.blocks import get_restate_blocks
from payserai.payseraibot.slack.constants import SLACK_CHANNEL_ID
...
from payserai.payseraibot.slack.utils import fetch_userids_from_emails
from payserai.payseraibot.slack.utils import respond_in_thread
from payserai.db.engine import get_sqlalchemy_engine
from payserai.db.models import SlackBotConfig
...
def handle_message(
    ...
    client: WebClient,
    num_retries: int = PAYSERAI_BOT_NUM_RETRIES,
    answer_generation_timeout: int = PAYSERAI_BOT_ANSWER_GENERATION_TIMEOUT,
    should_respond_with_error_msgs: bool = PAYSERAI_BOT_DISPLAY_ERROR_MSGS,
    disable_docs_only_answer: bool = PAYSERAI_BOT_DISABLE_DOCS_ONLY_ANSWER,
    disable_auto_detect_filters: bool = DISABLE_PAYSERAI_BOT_FILTER_DETECT,
    reflexion: bool = ENABLE_PAYSERAIBOT_REFLEXION,
) -> bool:
    ...
    payserai failing to answer/retrieve docs does count and should be notified
    """
    ...
    if respond_tag_only and not bipass_filters:
        logger.info(
            ...
            "payseraiBot only responds to tags"
        )
        return False
    ...
    # If configured to respond to team members only, then cannot be used with a /payseraiBot command
    ...
    if respond_team_member_list and is_bot_msg:
        if sender_id:
            respond_in_thread(
                ...
                text="The payseraiBot slash command is not enabled for this channel",
                thread_ts=None,
            )
    ...
    if not answer.answer and disable_docs_only_answer:
        logger.info(
            ...
            "`payserai_BOT_DISABLE_DOCS_ONLY_ANSWER` env variable is set"
        )
        return True

    # If called with the payseraiBot slash command, the question is lost so we have to reshow it
    ...
    try:
        respond_in_thread(
            ...
            text="Hello! payserai has some results for you!",
