...
from payserai.chat.chat_prompts import build_combined_query
from payserai.chat.chat_prompts import PAYSERAI_TOOL_NAME
from payserai.chat.chat_prompts import form_require_search_text
from payserai.chat.chat_prompts import form_tool_followup_text
from payserai.chat.chat_prompts import form_tool_less_followup_text
from payserai.chat.chat_prompts import form_tool_section_text
from payserai.chat.chat_prompts import form_user_prompt_text
from payserai.chat.chat_prompts import format_payserai_chunks_for_chat
from payserai.chat.chat_prompts import REQUIRE_PAYSERAI_SYSTEM_MSG
from payserai.chat.chat_prompts import YES_SEARCH
from payserai.chat.personas import build_system_text_from_persona
from payserai.chat.tools import call_tool
from payserai.configs.app_configs import NUM_DOCUMENT_TOKENS_FED_TO_CHAT
from payserai.configs.chat_configs import FORCE_TOOL_PROMPT
...
from payserai.direct_qa.interfaces import payseraiChatModelOut
from payserai.direct_qa.interfaces import StreamingError
from payserai.direct_qa.qa_utils import get_usable_chunks
...
from payserai.server.models import RetrievalDocs
from payserai.utils.logger import setup_logger
from payserai.utils.text_processing import extract_embedded_json
from payserai.utils.text_processing import has_unescaped_quote
...
def _parse_embedded_json_streamed_response(
    tokens: Iterator[str],
) -> Iterator[payseraiAnswerPiece | payseraiChatModelOut]:
    final_answer = False
    ...
    for token in tokens:
        ...
        if final_answer and '"actioninput":"' in model_output.lower().replace(
            ...
            if finding_end <= 1:
                ...
                yield payseraiAnswerPiece(answer_piece=hold)
                hold = ""
...
) -> Iterator[payseraiAnswerPiece | StreamingError]:
    try:
        prompt_msgs = [translate_payserai_msg_to_langchain(msg) for msg in messages]
        ...
        for token in get_default_llm().stream(all_msgs):
            yield payseraiAnswerPiece(answer_piece=token)
...
def llm_contextual_chat_answer(
    ...
    tokenizer: Callable,
    db_session: Session,
    run_search_system_text: str = REQUIRE_PAYSERAI_SYSTEM_MSG,
) -> Iterator[payseraiAnswerPiece | RetrievalDocs | StreamingError]:
    last_message = messages[-1]
    final_query_text = last_message.message
    previous_messages = messages[:-1]
    previous_msgs_as_basemessage = [
        translate_payserai_msg_to_langchain(msg) for msg in previous_messages
    ]
    ...
    try:
        ...
        payserai_system_tokens = len(tokenizer(run_search_system_text))
        ...
        need_search_prompt = _drop_messages_history_overflow(
            ...
            system_token_count=payserai_system_tokens,
        ...
        if (YES_SEARCH.split()[0] + " ").lower() in model_out.lower():
            ...
            retrieved_chunks = payserai_chat_retrieval(
            ...
            tool_result_str = format_payserai_chunks_for_chat(retrieved_chunks)
        ...
        for segment in extract_citations_from_stream(tokens, links):
            yield payseraiAnswerPiece(answer_piece=segment)
...
def llm_tools_enabled_chat_answer(
    ...
    try:
        ...
        final_result: payseraiChatModelOut | None = None
        final_answer_streamed = False

        for result in _parse_embedded_json_streamed_response(tokens):
            if isinstance(result, payseraiAnswerPiece) and result.answer_piece:
                yield result
                final_answer_streamed = True

            if isinstance(result, payseraiChatModelOut):
                final_result = result
                break
        ...
        if (
            retrieval_enabled
            and final_result.action.lower() == PAYSERAI_TOOL_NAME.lower()
        ):
            user_acl_filters = build_access_filters_for_user(user, db_session)
            doc_set_filter = [doc_set.name for doc_set in persona.document_sets] or None
...
def llm_chat_answer(
    ...
    # No additional tools outside of payserai retrieval, can use a more basic prompt
