...
def format_payserai_chunks_for_chat(chunks: list[InferenceChunk]) -> str:
    if not chunks:
...
def build_combined_query(
    ...
    combined_query_msgs.append(SystemMessage(content=payserai_SYSTEM_MSG))

    combined_query_msgs.extend(
        [translate_payserai_msg_to_langchain(msg) for msg in history]
    )
