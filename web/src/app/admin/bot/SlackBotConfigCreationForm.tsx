          ...
          <Formik
            ...
            onSubmit={async (values, formikHelpers) => {
              ...
              if (response.ok) {
                setPopup({
                  message: isUpdate
                    ? "Successfully updated payseraiBot config!"
                    : "Successfully created payseraiBot config!",
                  type: "success",
                });
                onClose();
              } else {
                ...
                setPopup({
                  message: isUpdate
                    ? `Error updating payseraiBot config - ${errorMsg}`
                    : `Error creating payseraiBot config - ${errorMsg}`,
                  type: "error",
                });
              }
            }}
            ...
            {({ isSubmitting, values }) => (
              <Form>
                <h2 className="text-xl font-bold mb-3 border-b border-gray-600 pt-4 pb-3 bg-gray-700 px-6">
                  {isUpdate
                    ? "Update a payseraiBot Config"
                    : "Create a new payseraiBot Config"}
                </h2>
                <div className="px-6 pb-6">
                  <TextArrayField
                    ...
                    subtext={
                      <div>
                        ...
                        <br />
                        <br />
                        <i>NOTE</i>: you still need to add payseraiBot to the
                  ...
                  <BooleanFormField
                    ...
                    label="Respond to @payseraiBot Only"
                    subtext="If set, payseraiBot will only respond when directly tagged"
                  />
                  <div className="border-t border-gray-600 py-2" />
                  <TextArrayField
                    name="respond_team_member_list"
                    label="Team Members Emails:"
                    subtext={`If specified, payseraiBot responses will only be 
                  visible to members in this list. This is
                  useful if you want payseraiBot to operate in an
                  "assistant" mode, where it helps the team members find
                  answers, but let's them build on top of payseraiBot's response / throw 
                      ...
                      <div>
                        <div>
                          ...
                          <div className="text-xs">
                            The document sets that payseraiBot should search
                            through. If left blank, payseraiBot will search
